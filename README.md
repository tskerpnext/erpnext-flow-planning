...
├── 0501_ERPNext備份與還原流程說明.md
├── 0505_ERPNext單據無法刪除或取消解決方法.md
├── 0505_ERPNext標準銷售出貨流程指南.md
├── 待確認事項/
│   ├── 0422_銷售模組流程與月結對帳自動化規劃.md
│   └── 0512_ERPNext導入規劃時程.md
└── 遷移及升級/
...
| [0501_ERPNext備份與還原流程說明](0501_ERPNext備份與還原流程說明.md) | 兩種備份差異（自訂設定 vs 完整站台）、建立備份與還原指令（含 custom_doctypes）、測試新功能前的標準流程，對應 erpnext-backups repo | 2026-05-01 |
| [0430_ERPNext差異化領料策略設定指南](0430_ERPNext差異化領料策略設定指南.md) | 不同產品線（電纜品/加工品）差異化領料與超領管理策略：系統就寬設定（Allow Over-delivery/receipt）配合雙套 SOP + 用戶權限分工，確保成本核算準確性 | 2026-04-30 |
| [0505_ERPNext標準銷售出貨流程指南](0505_ERPNext標準銷售出貨流程指南.md) | 職責分離模式的標準銷售出貨 SOP：業務員建交貨單草稿、倉管員提交確認出庫；含角色權限配置表（Stock User 須補 Address / Serial No / Batch 讀取權）與詳細操作步驟 | 2026-05-05 |
...
| [0512_ERPNext導入規劃時程](待確認事項/0512_ERPNext導入規劃時程.md) | 電纜+加工雙產品線製造業（約188人）的 ERPNext v16 導入時程規劃，含五大模組範圍、核心目標（帳料符合/生產不報工/列印格式）與各階段里程碑；工期壓縮為約6個月（不聘外部顧問，內部自行實施） | 2026-05-12 |
